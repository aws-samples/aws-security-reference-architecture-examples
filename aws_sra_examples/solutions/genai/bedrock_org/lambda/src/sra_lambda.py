...
class SRALambda:
    """Class to setup SRA Lambda resources in the organization."""
    ...
    def create_lambda_function(  # noqa: CFQ002, CCR001
        ...
        Args:
            code_zip_file: Lambda function code zip file
            role_arn: Lambda function role arn
            function_name: Lambda function name
            handler: Lambda function handler
            runtime: Lambda function runtime
            timeout: Lambda function timeout
            memory_size: Lambda function memory size
            solution_name: SRA solution name

        Raises:
            ValueError: Unexpected error executing Lambda function

        Returns:
            Lambda function arn if created, else "None"
        """
                ...
                if error.response["Error"]["Code"] == "ResourceConflictException":
                    try:
                        ...
                        update_response = self.LAMBDA_CLIENT.update_function_code(
                            FunctionName=function_name,
                            ZipFile=open(code_zip_file, "rb").read(),  # noqa: SIM115
                        )
    ...
    def get_permissions(self, function_name: str) -> str:
        ...
        Returns:
            Lambda function permissions if found, else "None"
        """
        try:
    ...
    def put_permissions(self, function_name: str, statement_id: str, principal: str, action: str, source_arn: str) -> str:
        ...
        Returns:
            str: Lambda function permissions if found, else "None"
        """
        try:
    ...
    def put_permissions_acct(self, function_name: str, statement_id: str, principal: str, action: str, source_acct: str) -> str:
        ...
        Args:
            ...
            principal (str): Principal
            action (str): Action
            source_acct (str): Source Account
